...
# Dataset file
authoritative_dataset_filename = "Datasets/Valov_system01.csv"

# Columns containing the function domain
domain_columns = [
"extreme","sparse","crc32","crc64","none","sha256","-9","-8","-7","-6","-5","-4","-3","-2","-1","0"
]
...
# -----------------------------------------------------------------------------------------------------------------
#
# APP Scenario 2 - Predicting Application Performance with hardware change
# Application: XZ
# Configuration: 75 to 80
#
# -----------------------------------------------------------------------------------------------------------------
...
Application_Experiment_2a = ModelMapExperiment.ModelMapExperiment(
...
direct_models = [
                ...
                POL(degree=4),
                LINSVR(random_state=seed),
                GPyR(GPy.kern.Matern52(input_dim=16, variance=1.0, lengthscale=1.0)),
                GPyMT(GPy.kern.Matern52(input_dim=16, variance=1.0, lengthscale=1.0)),
                CHOR(Chorus_config),
            ]

fmap = ModelMapMap.ModelMapMapDifferenceL_1(auth_model, map_models, ["0", "-3"])

Application_Experiment_2b = ModelMapExperiment.ModelMapExperiment(
    authoritative_dataset_filename=authoritative_dataset_filename,
    domain_columns=domain_columns,
    codomain_columns=codomain_columns,
    num_runs=num_runs,
    ...
    model_selection=False,
    logger=logger,
    title="APP-Scenario2b-"
    )

Application_Experiment_2 = ModelMapExperiment.ModelMapCompositeExperiment(
    "APP-Scenario2-",
    [Application_Experiment_2a,
     Application_Experiment_2b,
    ])

Application_Experiment_2.run()