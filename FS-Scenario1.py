...
domain_columns = ["workload_id", "machine_id", "io_scheduler", "block_size", "bg_count", "inode_size"]
...
direct_models = [
                ...
                POL(degree=4),
                LINSVR(random_state=seed),
                GPyR(GPy.kern.Matern52(input_dim=6, variance=1.0, lengthscale=1.0)),
                GPyMT(GPy.kern.Matern52(input_dim=6, variance=1.0, lengthscale=1.0)),
                CHOR(Chorus_config),
            ]

fmap = ModelMapMap.ModelMapMapDifferenceL_1(auth_model, map_models, ["inode_size", "bg_count"])

# -----------------------------------------------------------------------------------------------------------------
#
# Filesystem Scenario 1 - Modeling the effects of changing disk type (SATA -> 500SATA)
#
# -----------------------------------------------------------------------------------------------------------------

Filesystem_Experiment_1 = ModelMapExperiment.ModelMapExperiment(
